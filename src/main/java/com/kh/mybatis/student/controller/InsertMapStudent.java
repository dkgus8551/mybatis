...
public class InsertMapStudent extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = new StudentServiceImpl().insertStudent(param);
		
		if(result > 0) {
			request.getSession().setAttribute("msg", "Map 형식으로 데이터 등록 성공");
		}else{
			request.getSession().setAttribute("msg", "Map 형식으로 데이터 등록 실패");
		}
